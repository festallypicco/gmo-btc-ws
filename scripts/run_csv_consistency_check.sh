...
LOCK_PATH="$RUNTIME_DIR/check_csv_db_consistency.lock"
LOG_DIR="$PROJECT_ROOT/log"
LOG_PATH="$LOG_DIR/csv_db_consistency_$(date +%Y-%m-%d).log"
CHECK_SCRIPT="$SCRIPT_DIR/check_csv_db_consistency.py"
PYTHON_BIN="$PROJECT_ROOT/.venv-host/bin/python"
LOCK_STALE_SEC=1800
WRAPPER_NAME="run_csv_consistency_check.sh"
...
log() {
    ...
    local line="[$timestamp] [$level] [csv_db_consistency] $message"
    echo "$line" >> "$LOG_PATH"
    echo "$line"
}
...
if [ "$exit_code" -eq 0 ]; then
    ...
    log "INFO" "Running check_csv_db_consistency.py"
    run_and_log "$CHECK_SCRIPT"
    python_exit=$?
    if [ "$python_exit" -eq 0 ]; then
        log "INFO" "check_csv_db_consistency.py completed successfully"
        exit_code=0
    else
        log "ERROR" "ERROR: check_csv_db_consistency.py failed (exit=$python_exit)"
        send_wrapper_telegram_alert \
            "$WRAPPER_NAME" \
            "check_csv_db_consistency.py exit=$python_exit"
        exit_code=1
    fi
fi
