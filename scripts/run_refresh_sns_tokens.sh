...
LOCK_PATH="$SCRIPT_DIR/run_refresh_sns_tokens.lock"
LOG_DIR="$PROJECT_ROOT/log"
LOG_PATH="$LOG_DIR/refresh_sns_tokens_$(date +%Y-%m-%d).log"
CHECK_SCRIPT="$SCRIPT_DIR/refresh_sns_tokens.py"
PYTHON_BIN="$PROJECT_ROOT/.venv-host/bin/python"
LOCK_STALE_SEC=1800
WRAPPER_NAME="run_refresh_sns_tokens.sh"
...
log() {
    ...
    local line="[$timestamp] [$level] [refresh_sns_tokens] $message"
    echo "$line" >> "$LOG_PATH"
    echo "$line"
}
...
if [ "$exit_code" -eq 0 ]; then
    ...
    log "INFO" "Running refresh_sns_tokens.py"
    run_and_log "$CHECK_SCRIPT"
    python_exit=$?
    if [ "$python_exit" -eq 0 ]; then
        log "INFO" "refresh_sns_tokens.py completed successfully"
        exit_code=0
    else
        log "ERROR" "ERROR: refresh_sns_tokens.py failed (exit=$python_exit)"
        send_wrapper_telegram_alert \
            "$WRAPPER_NAME" \
            "refresh_sns_tokens.py exit=$python_exit"
        exit_code=1
    fi
fi
