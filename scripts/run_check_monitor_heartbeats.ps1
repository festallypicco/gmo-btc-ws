...
$LockPath = Join-Path $ScriptDir "run_check_monitor_heartbeats.lock"
$LogDir = Join-Path $ProjectRoot "log"
$LogPath = Join-Path $LogDir ("monitor_heartbeats_{0}.log" -f (Get-Date -Format "yyyy-MM-dd"))
$CheckScriptPath = Join-Path $ScriptDir "check_monitor_heartbeats.py"

function Write-MonitorHeartbeatsLog {
    param(
        [Parameter(Mandatory = $true)][string]$Message,
        [ValidateSet("INFO", "WARN", "ERROR")][string]$Level = "INFO"
    )
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $line = "[{0}] [{1}] [monitor_heartbeats] {2}" -f $timestamp, $Level, $Message
...
function Send-WrapperTelegramAlert {
    ...
    try {
        ...
        Write-MonitorHeartbeatsLog -Message ("Telegram notify failed in catch: {0}" -f $_.Exception.Message) -Level "WARN"
    }
    finally {
...
try {
    ...
    if (Test-Path -Path $LockPath) {
        ...
        if ($lockAge.TotalMinutes -le 30) {
            Write-MonitorHeartbeatsLog -Message "Lock file is fresh (<=30m). Another run may still be active; skipping." -Level "WARN"
            $exitCode = 1
        }
        else {
            Write-MonitorHeartbeatsLog -Message ("Removing stale lock file: {0}" -f $LockPath) -Level "WARN"
    ...
    if ($exitCode -eq 0) {
        ...
        Write-MonitorHeartbeatsLog -Message "Running check_monitor_heartbeats.py" -Level "INFO"
        $pythonExitCode = Invoke-PythonScriptAndLog -ScriptPath $CheckScriptPath
        if ($pythonExitCode -eq 0) {
            Write-MonitorHeartbeatsLog -Message "check_monitor_heartbeats.py completed successfully" -Level "INFO"
            $exitCode = 0
        }
        else {
            Write-MonitorHeartbeatsLog -Message ("ERROR: check_monitor_heartbeats.py failed (exit={0})" -f $pythonExitCode) -Level "ERROR"
            Send-WrapperTelegramAlert `
                -WrapperName "run_check_monitor_heartbeats.ps1" `
                -ErrorText ("check_monitor_heartbeats.py exit={0}" -f $pythonExitCode)
            $exitCode = 1
        }
    }
}
catch {
    Write-MonitorHeartbeatsLog -Message ("ERROR: run_check_monitor_heartbeats.ps1 exception: {0}" -f $_.Exception.Message) -Level "ERROR"
    Send-WrapperTelegramAlert -WrapperName "run_check_monitor_heartbeats.ps1" -ErrorText $_.Exception.Message
    $exitCode = 1
}
finally {
    if ($lockAcquired -and (Test-Path -Path $LockPath)) {
        Remove-Item -Path $LockPath -Force -ErrorAction SilentlyContinue
        Write-MonitorHeartbeatsLog -Message "Lock file removed." -Level "INFO"
    }
}
