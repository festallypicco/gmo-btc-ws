...
$LockPath = Join-Path $RuntimeDir "build_daily_report.lock"
$LogDir = Join-Path $ProjectRoot "log"
$LogPath = Join-Path $LogDir ("daily_report_{0}.log" -f (Get-Date -Format "yyyy-MM-dd"))
$CheckScriptPath = Join-Path $ScriptDir "build_daily_report.py"

function Write-DailyReportLog {
    param(
        [Parameter(Mandatory = $true)][string]$Message,
        [ValidateSet("INFO", "WARN", "ERROR")][string]$Level = "INFO"
    )
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $line = "[{0}] [{1}] [daily_report] {2}" -f $timestamp, $Level, $Message
...
function Send-WrapperTelegramAlert {
    ...
    try {
        ...
        Write-DailyReportLog -Message ("Telegram notify failed in catch: {0}" -f $_.Exception.Message) -Level "WARN"
    }
    finally {
...
try {
    ...
    if (Test-Path -Path $LockPath) {
        ...
        if ($lockAge.TotalMinutes -le 30) {
            Write-DailyReportLog -Message "Lock file is fresh (<=30m). Another run may still be active; skipping." -Level "WARN"
            $exitCode = 1
        }
        else {
            Write-DailyReportLog -Message ("Removing stale lock file: {0}" -f $LockPath) -Level "WARN"
    ...
    if ($exitCode -eq 0) {
        ...
        Write-DailyReportLog -Message "Running build_daily_report.py" -Level "INFO"
        $pythonExitCode = Invoke-PythonScriptAndLog -ScriptPath $CheckScriptPath
        if ($pythonExitCode -eq 0) {
            Write-DailyReportLog -Message "build_daily_report.py completed successfully" -Level "INFO"
            $exitCode = 0
        }
        else {
            Write-DailyReportLog -Message ("ERROR: build_daily_report.py failed (exit={0})" -f $pythonExitCode) -Level "ERROR"
            Send-WrapperTelegramAlert `
                -WrapperName "run_daily_report.ps1" `
                -ErrorText ("build_daily_report.py exit={0}" -f $pythonExitCode)
            $exitCode = 1
        }
    }
}
catch {
    Write-DailyReportLog -Message ("ERROR: run_daily_report.ps1 exception: {0}" -f $_.Exception.Message) -Level "ERROR"
    Send-WrapperTelegramAlert -WrapperName "run_daily_report.ps1" -ErrorText $_.Exception.Message
    $exitCode = 1
}
finally {
    if ($lockAcquired -and (Test-Path -Path $LockPath)) {
        Remove-Item -Path $LockPath -Force -ErrorAction SilentlyContinue
        Write-DailyReportLog -Message "Lock file removed." -Level "INFO"
    }
}
