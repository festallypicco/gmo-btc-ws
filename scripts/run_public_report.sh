...
LOCK_PATH="$SCRIPT_DIR/run_public_report.lock"
LOG_DIR="$PROJECT_ROOT/log"
LOG_PATH="$LOG_DIR/public_report_$(date +%Y-%m-%d).log"
CHECK_SCRIPT="$SCRIPT_DIR/build_public_report.py"
PYTHON_BIN="$PROJECT_ROOT/.venv-host/bin/python"
LOCK_STALE_SEC=1800
WRAPPER_NAME="run_public_report.sh"
...
log() {
    ...
    local line="[$timestamp] [$level] [public_report] $message"
    echo "$line" >> "$LOG_PATH"
    echo "$line"
}
...
if [ "$exit_code" -eq 0 ]; then
    ...
    log "INFO" "Running build_public_report.py"
    run_and_log "$CHECK_SCRIPT"
    python_exit=$?
    if [ "$python_exit" -eq 0 ]; then
        log "INFO" "build_public_report.py completed successfully"
        exit_code=0
    else
        log "ERROR" "ERROR: build_public_report.py failed (exit=$python_exit)"
        send_wrapper_telegram_alert \
            "$WRAPPER_NAME" \
            "build_public_report.py exit=$python_exit"
        exit_code=1
    fi
fi
