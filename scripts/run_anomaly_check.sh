...
LOCK_PATH="$SCRIPT_DIR/run_anomaly_check.lock"
LOG_DIR="$PROJECT_ROOT/log"
LOG_PATH="$LOG_DIR/anomaly_check_$(date +%Y-%m-%d).log"
CHECK_SCRIPT="$SCRIPT_DIR/check_trading_anomaly.py"
PYTHON_BIN="$PROJECT_ROOT/.venv-host/bin/python"
LOCK_STALE_SEC=1800
WRAPPER_NAME="run_anomaly_check.sh"
...
log() {
    ...
    local line="[$timestamp] [$level] [anomaly_check] $message"
    echo "$line" >> "$LOG_PATH"
    echo "$line"
}
...
if [ "$exit_code" -eq 0 ]; then
    ...
    log "INFO" "Running check_trading_anomaly.py"
    run_and_log "$CHECK_SCRIPT"
    python_exit=$?
    if [ "$python_exit" -eq 0 ]; then
        log "INFO" "check_trading_anomaly.py completed successfully"
        exit_code=0
    else
        log "ERROR" "ERROR: check_trading_anomaly.py failed (exit=$python_exit)"
        send_wrapper_telegram_alert \
            "$WRAPPER_NAME" \
            "check_trading_anomaly.py exit=$python_exit"
        exit_code=1
    fi
fi
