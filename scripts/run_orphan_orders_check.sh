...
LOCK_PATH="$SCRIPT_DIR/run_orphan_orders_check.lock"
...
log() {
    ...
    local line="[$timestamp] [$level] [orphan_orders_check] $message"
    echo "$line" >> "$LOG_PATH"
    echo "$line"
}
...
if [ "$exit_code" -eq 0 ]; then
    ...
    log "INFO" "Running check_orphan_orders.py"
    run_and_log "$CHECK_SCRIPT"
    python_exit=$?
    if [ "$python_exit" -eq 0 ]; then
        log "INFO" "check_orphan_orders.py completed successfully"
        exit_code=0
    else
        log "ERROR" "ERROR: check_orphan_orders.py failed (exit=$python_exit)"
        send_wrapper_telegram_alert \
            "$WRAPPER_NAME" \
            "check_orphan_orders.py exit=$python_exit"
        exit_code=1
    fi
fi
